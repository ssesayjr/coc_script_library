...
def main():
    ...
    if not rs.table_exists(user_table): #if the table doesn't exist create it
       rs.query(f"create table {user_table} (orgname varchar(2048), email varchar(2048), reportid varchar(2048), date_range_begin timestamp,date_range_end timestamp, domain varchar(1024), adkim varchar(1024), aspf varchar(1024), p varchar(1024), sp timestamp, pct varchar(1024), source_ip varchar(1024), disposition varchar(1024), dkim varchar(1024), spf varchar(1024));")
    ...
    if len(keys) == 0:
        ...
        for x in files:
            ...
            try:
                ...
                rs.copy(final_table,user_table,truncatecolumns=True,columntypes={'orgname': 'varchar(2048)', 'email': 'varchar(2048)', 'reportid': 'varchar(2048)', 'date_range_begin': 'timestamp','date_range_end': 'timestamp', 'domain': 'varchar(1024)', 'adkim': 'varchar(1024)', 'aspf': 'varchar(1024)', 'p': 'varchar(1024)', 'sp': 'timestamp', 'pct': 'varchar(1024)', 'source_ip': 'varchar(1024)', 'disposition': 'varchar(1024)', 'dkim': 'varchar(1024)', 'spf': 'varchar(1024)'},temp_bucket_region= 'us-west-2',if_exists='append')
            except Exception:
                rs.copy(final_table,user_table,truncatecolumns=True,columntypes={'orgname': 'varchar(2048)', 'email': 'varchar(2048)', 'reportid': 'varchar(2048)', 'date_range_begin': 'timestamp','date_range_end': 'timestamp', 'domain': 'varchar(1024)', 'adkim': 'varchar(1024)', 'aspf': 'varchar(1024)', 'p': 'varchar(1024)', 'sp': 'timestamp', 'pct': 'varchar(1024)', 'source_ip': 'varchar(1024)', 'disposition': 'varchar(1024)', 'dkim': 'varchar(1024)', 'spf': 'varchar(1024)'},temp_bucket_region= 'us-west-2',if_exists='drop')
